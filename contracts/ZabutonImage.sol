...
contract ZabutonImage is IZabutonImage, Ownable {
    ...
    function getImage(uint256 num) public view returns (string memory) {
        ...
        image = string.concat(svgHeader, zabuton, backFrame);
        ...
        image = string.concat(image, foreFrame, "</svg>");
        return image;
    }
    ...
    function getForeFrame() public view onlyOwner returns (string memory) {
        return foreFrame;
    }

    function setForeFrame(string memory _frame) public onlyOwner {
        foreFrame = _frame;
    }

    function getBackFrame() public view onlyOwner returns (string memory) {
        return backFrame;
    }

    function setBackFrame(string memory _frame) public onlyOwner {
        backFrame = _frame;
    }
}
